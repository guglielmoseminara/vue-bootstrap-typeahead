  ...
  computed: {
    highlight() {
      return (text) => {
        ...
        const splittedQuery = this.escapedQuery.split(' ');
        for (let sq in splittedQuery) {
          if (splittedQuery[sq].length > 0) {
            highlightedText = highlightedText.replace(new RegExp('('+splittedQuery[sq]+')(?![^<]*>|[^<>]*</)', 'gi'), `<strong>$&</strong>`);
          }
        }
